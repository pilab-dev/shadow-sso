...
func TestBBoltStore_EnsureBucket(t *testing.T) {
	store, dbPath, cleanup := setupTestDB(t)
	defer cleanup()

	// Close the initial store to work directly with bbolt for verification
	...
	// Re-create store to call ensureBucket (it's called by Set)
	storeToTest, err := NewBBoltStore(dbPath, 1*time.Hour, 10*time.Minute)
	require.NoError(t, err)
	defer storeToTest.Close()

	// ensureBucket is private, but Set calls it.
	err = storeToTest.Set(bucketName, "dummykey", []byte("dummy"), 0)
	require.NoError(t, err)
