		...
		if cErr != nil {
			...
			if hashErr != nil {
				...
				if createErr := clientRepo.CreateClient(bCtx, client); createErr != nil {
					...
					// The plaintext secret is never persisted (only its bcrypt hash is),
					// so this is the one and only chance to hand it to the operator.
					// Write it to a restrictive-permission file instead of logging it,
					// since logs commonly end up in aggregators with broader access.
					const bootstrapSecretPath = "/etc/sso/admin-bootstrap-client-secret"
					if writeErr := os.WriteFile(bootstrapSecretPath, []byte(clientSecret), 0o600); writeErr != nil {
						log.Warn().Err(writeErr).Str("client_id", "admin-ui").
							Msg("Admin UI OAuth client created with an auto-generated secret, but failed to write it to " + bootstrapSecretPath + " — set SSSO_INITIAL_ADMIN_CLIENT_SECRET explicitly instead")
					} else {
						log.Warn().Str("client_id", "admin-ui").Str("secret_path", bootstrapSecretPath).
							Msg("Admin UI OAuth client created with an auto-generated secret — read it once from secret_path, then delete the file and rotate the secret for production use")
					}
				} else {
					log.Info().Str("client_id", "admin-ui").Msg("Admin UI OAuth client created using the configured SSSO_INITIAL_ADMIN_CLIENT_SECRET")
				}
			}
		} else {
