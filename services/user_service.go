...
// PasswordHasher interface (assuming it's defined elsewhere or provided)
type PasswordHasher interface {
	Hash(password string) (string, error)
	Verify(hashedPassword, password string) bool
}

// --- Interface for the new UserService ---
type UserServiceInternal interface {
	...
	ListUsers(ctx context.Context, // Add pagination/filter DTOs if needed
	) ([]*dto.UserResponse, error) // Simplified for now
	...
	ChangePassword(ctx context.Context, userID string, oldPassword string, newPassword string) error // Simplified
...
func NewUserServiceImpl(userRepo domain.UserRepository, hasher PasswordHasher) *UserServiceImpl {
...
func (s *UserServiceImpl) CreateUser(ctx context.Context, req *dto.UserCreateRequest) (*dto.UserResponse, error) {
	if req.Password == "" {
		return nil, errors.New("password is required") // Basic validation
	}
	// Check if user already exists
	if _, err := s.userRepo.GetUserByEmail(ctx, req.Email); err == nil {
		return nil, fmt.Errorf("user with email '%s' already exists", req.Email) // Consider custom error
	}
	...
	domainUser := dto.ToDomainUser(*req) // This mapper needs to be aware of PasswordHash field
	domainUser.PasswordHash = hashedPassword
	domainUser.Status = domain.UserStatusActive // Default status, or PENDING_ACTIVATION if email verification is a flow

	if err := s.userRepo.CreateUser(ctx, domainUser); err != nil {
		log.Error().Err(err).Msg("Failed to create user in service")
		return nil, fmt.Errorf("failed to create user: %w", err)
	}
	return dto.FromDomainUser(domainUser), nil
}

func (s *UserServiceImpl) GetUserByID(ctx context.Context, userID string) (*dto.UserResponse, error) {
	// Start a custom span
	...
	domainUser, err := s.userRepo.GetUserByID(newCtx, userID) // Pass newCtx
	if err != nil {
...
func (s *UserServiceImpl) GetUserByEmail(ctx context.Context, email string) (*dto.UserResponse, error) {
	domainUser, err := s.userRepo.GetUserByEmail(ctx, email)
	if err != nil {
...
func (s *UserServiceImpl) ListUsers(ctx context.Context) ([]*dto.UserResponse, error) {
	// Placeholder: Real implementation would need pagination and filtering
	domainUsers, err := s.userRepo.ListUsers(ctx, 0, 0, "", "") // Assuming ListUsers takes limit, offset, filter, sort
	if err != nil {
...
func (s *UserServiceImpl) UpdateUser(ctx context.Context, userID string, req *dto.UserUpdateRequest) (*dto.UserResponse, error) {
	...
	if req.Email != nil {
		...
		if otherUser, errEmail := s.userRepo.GetUserByEmail(ctx, *req.Email); errEmail == nil && otherUser.ID != userID {
...
func (s *UserServiceImpl) LockUser(ctx context.Context, userID string) error {
	user, err := s.userRepo.GetUserByID(ctx, userID)
	if err != nil {
		return fmt.Errorf("user not found: %w", err)
	}
...
func NewUserServer(service UserServiceInternal) *UserServer {
	return &UserServer{
		service: service,
	}
}
...
func dtoUserResponseToProto(userResp *dto.UserResponse) *ssov1.User {
	...
	protoUser := &ssov1.User{
		...
		Status:             string(userResp.Status), // Assuming ssov1.User.Status is string
	}
...
func (s *UserServer) RegisterUser(ctx context.Context, req *connect.Request[ssov1.RegisterUserRequest]) (*connect.Response[ssov1.RegisterUserResponse], error) {
	dtoReq := &dto.UserCreateRequest{
		...
		Roles:     req.Msg.Roles, // Assuming proto roles directly map
	}
...
func (s *UserServer) GetUser(ctx context.Context, req *connect.Request[ssov1.GetUserRequest]) (*connect.Response[ssov1.GetUserResponse], error) {
	...
	} else if req.Msg.GetEmail() != "" {
		dtoResp, err = s.service.GetUserByEmail(ctx, req.Msg.GetEmail())
	} else {
