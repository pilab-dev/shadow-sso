...
func (p *DefaultServiceProvider) JWKSService() *JWKSService {
	if p.jwksService == nil {
		...
		var err error
		p.jwksService, err = NewJWKSService(p.config.JWKSDefaultKeyRotationInterval) // Default from config
		if err != nil {
...
func (p *DefaultServiceProvider) UserService() *UserService {
	if p.userService == nil {
		// UserService dependencies: userRepo, passwordHasher
		p.userService = NewUserService(
			p.repoProvider.UserRepository(initCtx),
			p.PasswordHasher(), // Get PasswordHasher via its getter
		)
	}
	return p.userService
}

func (p *DefaultServiceProvider) AuthService() *AuthService {
	if p.authService == nil {
		// AuthService dependencies: userRepo, sessionRepo, tokenService, passwordHasher, ?clientRepo
		// ClientRepo might be needed if auth involves client-specific logic not covered by tokenService.
		// Current NewAuthService: userRepo, sessionRepo, tokenService, passwordHasher, config
		p.authService = NewAuthService(
			p.repoProvider.UserRepository(initCtx),
			p.repoProvider.SessionRepository(initCtx),
			p.TokenService(),
			p.PasswordHasher(),
			p.config, // Pass OpenIDProviderConfig
		)
	}
...
func (p *DefaultServiceProvider) FederationService() *federation.Service {
	if p.federationService == nil {
		...
		p.federationService = federation.NewService(
			...
			p.ClientService(),
			p.TokenService(),
			p.config.Issuer,      // issuerURL
			callbackBaseURL, // callbackBaseURL
			p.config, // Pass full config for provider-specific details if needed by NewService
		)
	}
...
func (p *DefaultServiceProvider) IdPManagementService() *IdPManagementService {
	if p.idpManagementService == nil {
		// IdPManagementService dependencies: idpRepo
...
func (p *DefaultServiceProvider) ClientManagementService() *ClientManagementService {
	if p.clientManagementService == nil {
		// ClientManagementService dependencies: clientRepo, ?other services like Audit
		p.clientManagementService = NewClientManagementService(
			p.repoProvider.ClientRepository(initCtx),
			p.PasswordHasher(), // For hashing client secrets if needed by service
			// Add other dependencies if NewClientManagementService requires them
		)
	}
...
func (p *DefaultServiceProvider) ServiceAccountService() *ServiceAccountService {
	if p.serviceAccountService == nil {
		// ServiceAccountService dependencies: saRepo, pubKeyRepo, tokenService
...
func (p *DefaultServiceProvider) TwoFactorService() *TwoFactorService {
	if p.twoFactorService == nil {
		// TwoFactorService dependencies: userRepo, (config for issuer name for TOTP)
		// NewTwoFactorService(userRepo domain.UserRepository, totpIssuer string)
		totpIssuer := "ShadowSSO" // Default or from config: p.config.TOTPIssuerName
		if p.config.TOTPIssuerName != "" {
			totpIssuer = p.config.TOTPIssuerName
		}
