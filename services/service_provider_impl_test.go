...
func TestNewDefaultServiceProvider_Success(t *testing.T) {
	...
	mockRepoProvider := mock_domain.NewMockRepositoryProvider(ctrl)
	...
	appConfig := &ssso.OpenIDProviderConfig{IssuerURL: "http://issuer.com"}
...
func setupMockRepoProviderForServiceGetters(mockRepoProvider *mock_domain.MockRepositoryProvider, ctrl *gomock.Controller) {
	...
	mockRepoProvider.EXPECT().ClientRepository(gomock.Any()).Return(mock_domain.NewMockClientStore(ctrl)).AnyTimes()
...
func TestDefaultServiceProvider_Getters(t *testing.T) {
	...
	appConfig := &ssso.OpenIDProviderConfig{
		...
		JWKSDefaultKeyRotationInterval: time.Hour,
	...
	// AuthService
	authService1 := sp.AuthService()
	assert.NotNil(t, authService1)
	authService2 := sp.AuthService()
	assert.Same(t, authService1, authService2, "AuthService should be a singleton")
	...
	idpMgtService1 := sp.IdPManagementService()
	assert.NotNil(t, idpMgtService1)
	idpMgtService2 := sp.IdPManagementService()
	assert.Same(t, idpMgtService1, idpMgtService2, "IdPManagementService should be a singleton")

	// ClientManagementService
	clientMgtService1 := sp.ClientManagementService()
	assert.NotNil(t, clientMgtService1)
	clientMgtService2 := sp.ClientManagementService()
	assert.Same(t, clientMgtService1, clientMgtService2, "ClientManagementService should be a singleton")

	// ServiceAccountService
	saService1 := sp.ServiceAccountService()
	assert.NotNil(t, saService1)
	saService2 := sp.ServiceAccountService()
	assert.Same(t, saService1, saService2, "ServiceAccountService should be a singleton")

	// TwoFactorService
	tfService1 := sp.TwoFactorService()
	assert.NotNil(t, tfService1)
	tfService2 := sp.TwoFactorService()
	assert.Same(t, tfService1, tfService2, "TwoFactorService should be a singleton")
