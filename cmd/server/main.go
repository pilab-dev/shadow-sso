...
var tracerProvider *sdktrace.TracerProvider // Use concrete type for shutdown
...
func main() {
	...
	if err != nil { appLogger.Fatal(ctx, "Failed to initialize OAuthRepository", err, nil) }

	userRepo, err := mongodb.NewUserRepositoryMongo(ctx, db)
	if err != nil { appLogger.Fatal(ctx, "Failed to initialize UserRepository", err, nil) }

	idpRepo, err := mongodb.NewIdPRepositoryMongo(ctx, db)
	if err != nil { appLogger.Fatal(ctx, "Failed to initialize IdPRepository", err, nil) }

	sessionRepo, err := mongodb.NewSessionRepositoryMongo(ctx, db)
	if err != nil { appLogger.Fatal(ctx, "Failed to initialize SessionRepository", err, nil) }

	pubKeyRepo, err := mongodb.NewPublicKeyRepositoryMongo(db)
	if err != nil { appLogger.Fatal(ctx, "Failed to initialize PublicKeyRepository", err, nil) }

	saRepo, err := mongodb.NewServiceAccountRepositoryMongo(db)
	if err != nil { appLogger.Fatal(ctx, "Failed to initialize ServiceAccountRepository", err, nil) }

	// Services
	passwordHasher := auth.NewBcryptPasswordHasher(bcrypt.DefaultCost)

	tokenRepoAssertion, ok := oauthRepo.(ssso.TokenRepository)
	if !ok { appLogger.Fatal(ctx, "mongodb.OAuthRepository does not implement ssso.TokenRepository", nil, nil) }
	...
	userServiceInternal := services.NewUserServiceImpl(userRepo, passwordHasher)
	idpServiceInternal := services.NewIdPServiceImpl(idpRepo)
	...
	if !ok { appLogger.Fatal(ctx, "mongodb.OAuthRepository does not implement client.ClientStore", nil, nil) }
	clientSvc := client.NewClientService(clientStoreAssertion)
	pkceRepoAssertion, ok := oauthRepo.(ssso.PkceRepository)
	if !ok { appLogger.Fatal(ctx, "mongodb.OAuthRepository does not implement ssso.PkceRepository", nil, nil) }
	...
	grpcServer = server.NewGRPCServer(cfg, appLogger,
		// idpRPCHandler, // Example: This would require NewGRPCServer to accept ssov1connect.IdPManagementServiceHandler
		// userRPCHandler,
		// clientRPCHandler,
		// authRPCHandler,
		// twoFactorRPCHandler,
